        ...
        <div class="md:flex shadow-lg mx-6 md:mx-auto max-w-lg md:max-w-2xl">
          ...
          <div class="w-full md:w-2/3 px-4 py-4 bg-white rounded-lg">
            <div class="flex items-center">
              <h2 class="text-xl text-gray-800 font-medium mr-auto">{{ article.title }}</h2>
            </div>
            <p class="text-sm text-gray-700 mt-2">{{ article.description }}</p>
            ...
            <div class="meta">
              ...
              <time class="text-sm text-gray-700">{{ new Date(article.created_at).toLocaleDateString('en', { year: 'numeric', month: 'short', day: 'numeric' }) }}</time>
            </div>
          </div>
        </div>
      </nuxt-link>
